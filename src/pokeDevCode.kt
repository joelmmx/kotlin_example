...
fun main() {
    ...
    do{
        ...
        val ataqueAleatorio : Int = random.nextInt(pokemonSalvaje.listaAtaques.size)
...
fun procesarAtaque(pokemonAtacante: Pokemon, pokemonDefensor: Pokemon, ataqueSeleccionado: Int): Boolean {
    ...
    println("${pokemonDefensor.nombre} ha recibido $valorDanio puntos de danio!")
...
fun calcularDanio(valorAtaque: Int, valorDefensa: Int, ataque: Ataque): Int = (((2*1+10.0)/250*(valorAtaque/valorDefensa)*ataque.poder +2)*1.5).toInt()
...
fun generarPokemon(opcion: Int): Pokemon = when(opcion){
    Pokemones.BULBASAUR.id -> Pokemon("Bulbasaur",45,49, 49,arrayOf(Ataque("vine whip",45),Ataque("tacklet",40)))
    ...
    Pokemones.PIKACHU.id -> Pokemon("Pickachu",35,55, 40,arrayOf(Ataque("thunder shok",40),Ataque("tacklet",46)))
    else -> {
        Pokemon("Missingno",33,136,0, arrayOf(Ataque("pay day",20),Ataque("blind",15)))
    }
}
